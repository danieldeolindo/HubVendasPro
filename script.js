...
// SENHA_ADMIN removed - now uses user's own password hash for admin actions
...
/* ─── Chaves localStorage ─── */
const chaveProdutos   = e => `produtos_${e}`;
const chaveHistorico  = e => `historico_${e}`;
const chaveClientes   = e => `clientes_${e}`;
const chaveTema       = e => `tema_${e}`;
const chaveMenuOrdem  = e => `menuOrdem_${e}`;
const chaveLojaConfig = e => `lojaConfig_${e}`;
const chaveUsuarios   = ()  => `usuarios_hvp`;
...
const DB_NAME = "hvp_images"; let db = null;
function abrirDB() {
...
async function salvarImagem(k, b64) { try { const idb = await abrirDB(); return new Promise((res,rej) => { const tx = idb.transaction("images","readwrite"); tx.objectStore("images").put(b64,k).onsuccess = () => res(true); tx.onerror = () => rej(); }); } catch { return false; } }
async function carregarImagem(k) { try { const idb = await abrirDB(); return new Promise(res => { const req = idb.transaction("images","readonly").objectStore("images").get(k); req.onsuccess = () => res(req.result||null); req.onerror = () => res(null); }); } catch { return null; } }
async function removerImagem(k) { try { const idb = await abrirDB(); return new Promise(res => { idb.transaction("images","readwrite").objectStore("images").delete(k); res(true); }); } catch { return false; } }

/* ─── localStorage seguro ─── */
function lsGet(k, fb=null) { try { const v=localStorage.getItem(k); return v!==null?JSON.parse(v):fb; } catch { return fb; } }
function lsSet(k, v) { try { localStorage.setItem(k, JSON.stringify(v)); return true; } catch(e) { mostrarToast("⚠️ Erro ao salvar dados. Armazenamento cheio.","erro"); return false; } }
...
function renderCarrinho() {
  ...
  footerEl.innerHTML = `
    <div class="carrinho-subtotal-row">
      <span>Subtotal</span><span class="mono">R$ ${fmt(sub)}</span>
    </div>
    ${desc>0?`<div class="carrinho-subtotal-row desc"><span>Desconto</span><span class="mono red">− R$ ${fmt(desc)}</span></div>`:""}
    <div class="carrinho-subtotal-row total-row">
      <span>Total</span><span class="mono green">R$ ${fmt(tot)}</span>
    </div>`;
}

function removerDoCarrinho(id) { delete carrinho[id]; atualizarTotal(); renderProdutos(); }
function limparCarrinho() {
  carrinho = {}; atualizarTotal(); renderProdutos();
...
function renderSplitPagamento() {
  ...
  _renderSplitHTML(c, pagamentosSelecionados, splitPagamento, total, "onSplitInput", "splitAviso");
}
function onSplitInput(tipo) {
  const inp=document.getElementById(`split-input-${tipo}`);
  splitPagamento[tipo]=parseFloat(inp?.value)||0;
...
function renderEditSplit() {
  ...
  c.classList.remove("hidden");
  const total=_calcEditTotal();
  _renderSplitHTML(c, editarPagamentos, editarSplit, total, "onEditSplitInput", "editSplitAviso");
}
function onEditSplitInput(tipo) {
  const inp=document.getElementById(`split-input-${tipo}`);
  editarSplit[tipo]=parseFloat(inp?.value)||0;
  _atualizarResto(editarPagamentos, editarSplit, _calcEditTotal(), "editSplitAviso");
  atualizarEditarTotal();
}
...
function _atualizarResto(formas, splits, total, avisoId) {
  ...
  aviso.innerHTML=soma>total+.005
...
function getLojaConfig() { return lsGet(chaveLojaConfig(usuarioAtual?.email), {nome:"",cor:"#00bf63",fonte:"jakarta"}); }
function salvarLojaConfig() { if (usuarioAtual) lsSet(chaveLojaConfig(usuarioAtual.email), lojaConfigAtual); }
...
function getOrdemMenu() {
  if (!usuarioAtual) return MENU_PADRAO.map(m=>m.id);
  const s=lsGet(chaveMenuOrdem(usuarioAtual.email),null);
...
function salvarOrdemMenu(o) { if (usuarioAtual) lsSet(chaveMenuOrdem(usuarioAtual.email),o); }
function resetarOrdemMenu() { if (usuarioAtual) lsRemove(chaveMenuOrdem(usuarioAtual.email)); renderNavs(); renderMenuOrderList(); mostrarToast("Menu restaurado."); }
...
function login() {
  ...
  hashSenha(senha).then(hash=>{
    const user=lsGet(chaveUsuarios(),[]).find(u=>u.email===email&&u.senhaHash===hash);
    if (user){lsSet("usuarioLogado",{email:user.email,senhaHash:user.senhaHash});entrarNoPainel(user);}
    else mostrarToast("Email ou senha inválidos.","erro");
  });
}

function entrarNoPainel(user) {
  usuarioAtual=user; carrinho={}; pagamentosSelecionados=["dinheiro"]; splitPagamento={}; tipoDesconto="pct"; categoriaAtiva="todas"; filtroHistorico="hoje"; filtroRelatorio="hoje";
  produtos  = lsGet(chaveProdutos(user.email),[]);
  historico = lsGet(chaveHistorico(user.email),[]);
  clientes  = lsGet(chaveClientes(user.email),[]);
  document.getElementById("authScreen").classList.add("hidden");
  document.getElementById("appShell").classList.remove("hidden");
  carregarImagem(`avatar_${user.email}`).then(av=>atualizarExibicaoAvatar(av,user.email[0].toUpperCase()));
  document.getElementById("emailLogado").textContent=user.email;
  document.getElementById("total").innerText="0,00";
  document.documentElement.setAttribute("data-theme",lsGet(chaveTema(user.email),"light"));
  lojaConfigAtual=getLojaConfig(); aplicarNomeLoja();
...
function logout() {
  lsRemove("usuarioLogado"); usuarioAtual=null; carrinho={}; produtos=[]; historico=[]; clientes=[]; editandoId=null;
...
function registrar() {
  ...
  const users=lsGet(chaveUsuarios(),[]);
  if (users.find(u=>u.email===email)){mostrarToast("Email já cadastrado.","erro");return;}
  hashSenha(senha).then(hash=>{
    users.push({email,senhaHash:hash}); lsSet(chaveUsuarios(),users);
...
document.addEventListener("DOMContentLoaded",()=>{
  migrarSenhasAntigas();
  const s=lsGet("usuarioLogado",null);
  if (s) { const ok=lsGet(chaveUsuarios(),[]).find(u=>u.email===s.email&&u.senhaHash===s.senhaHash); if (ok) entrarNoPainel(ok); else lsRemove("usuarioLogado"); }
});

async function migrarSenhasAntigas() {
  const antigos=lsGet("usuarios",null); if (!antigos||!Array.isArray(antigos)) return;
  const ja=lsGet(chaveUsuarios(),[]), migrados=new Set(ja.map(u=>u.email)), novos=[];
  for (const u of antigos) { if (migrados.has(u.email)) continue; if (u.senhaHash) novos.push(u); else if (u.senha) novos.push({email:u.email,senhaHash:await hashSenha(u.senha)}); }
  if (novos.length) lsSet(chaveUsuarios(),[...ja,...novos]);
  lsRemove("usuarios");
}
...
function salvarCliente() {
  ...
  if (editandoClienteId!==null&&idx!==-1) {
    ...
    clientes.push({id:proximoIdCliente(),nome,telefone:tel,cpf,email,endereco:end});
    mostrarToast("✅ Cliente cadastrado!");
  }
  lsSet(chaveClientes(usuarioAtual.email),clientes);
...
function salvarProduto() {
  ...
  function salvar(fotoKey){
    const idx=produtos.findIndex(p=>p.id===editandoId);
    if (editandoId!==null&&idx!==-1){produtos[idx].nome=nome;produtos[idx].preco=preco;produtos[idx].categoria=cat;if(fotoKey)produtos[idx].fotoKey=fotoKey;editandoId=null;}
    else{produtos.push({id:Date.now(),skuId:proximoSkuId(),nome,preco,categoria:cat,fotoKey:fotoKey||""});}
    lsSet(chaveProdutos(usuarioAtual.email),produtos);
  ...
  if (fi.files[0]){const k=`foto_${usuarioAtual.email}_${Date.now()}`,r=new FileReader();r.onload=async e=>{await salvarImagem(k,e.target.result);salvar(k);};r.readAsDataURL(fi.files[0]);}
  else salvar(null);
}
...
async function excluirProduto(id){
  ...
  const p=produtos.find(p=>p.id===id);if(p?.fotoKey)await removerImagem(p.fotoKey);
  produtos=produtos.filter(p=>p.id!==id);delete carrinho[id];
  lsSet(chaveProdutos(usuarioAtual.email),produtos);atualizarTotal();renderProdutosAdmin();mostrarToast("Produto excluído.");
}
...
function finalizarPedido() {
  ...
  historico.push(venda);lsSet(chaveHistorico(usuarioAtual.email),historico);
...
function montarHTMLComprovante(venda) {
  ...
  if (venda.clienteId) {
    ...
    if (cli?.cpf) clienteLinha+=`<p class="cv-cliente">CPF: <strong>${cli.cpf}</strong></p>`;
  }
  return `<div class="cupom" style="background:#fff;color:#000;">
    ...
    <table class="cupom-itens">
      <thead><tr><th>Produto</th><th class="c">Qtd</th><th class="r">Valor</th></tr></thead>
...
function mostrarComprovante(venda) {
  vendaComprovanteAtual=venda;
  document.getElementById("comprovanteConteudo").innerHTML=montarHTMLComprovante(venda);
...
function imprimirComprovante() {
  ...
  w.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><style>
    body{font-family:'Courier New',monospace;font-size:13px;margin:0;padding:16px;background:#fff;color:#000;}
    .cupom{max-width:320px;margin:0 auto;}
    .cupom-header{text-align:center;margin-bottom:10px;}
    .cupom-logo{font-size:18px;font-weight:900;letter-spacing:-0.5px;}
    .cupom-sub{font-size:11px;color:#555;}
    .cupom-info{font-size:11px;color:#555;}
    .cv-cliente{font-size:12px;margin:4px 0 0;}
    .cupom-divider{text-align:center;font-size:11px;color:#aaa;margin:8px 0;}
    .cupom-itens{width:100%;border-collapse:collapse;font-size:12px;}
    .cupom-itens th{text-align:left;border-bottom:1px solid #ccc;padding-bottom:4px;}
    .cupom-itens td{padding:3px 0;}
    .cupom-itens .c,.cupom-itens th.c{text-align:center;}
    .cupom-itens .r,.cupom-itens th.r{text-align:right;}
    .cupom-totais{font-size:13px;}
    .cupom-linha{display:flex;justify-content:space-between;padding:3px 0;}
    .cupom-linha.desc{color:#c00;}
    .cupom-linha.total{font-weight:900;font-size:15px;border-top:1px solid #000;padding-top:6px;margin-top:2px;}
    .cupom-pagamento{font-size:12px;margin:6px 0;}
    .cupom-footer{text-align:center;font-size:11px;color:#888;margin-top:10px;}
    @media print{body{padding:0;}@page{margin:10mm 5mm;}}
  </style></head><body>${html}<script>window.onload=()=>{window.print();}<\/script></body></html>`);
  w.document.close();
}

function baixarComprovantePDF() {
  if (typeof window.jspdf==="undefined"&&typeof jspdf==="undefined"){mostrarToast("PDF indisponível. Use imprimir e salve como PDF.","erro");imprimirComprovante();return;}
  // Usa jsPDF via CDN
  ...
  v.itens.forEach(i=>{
    const sku=i.skuId?`#${String(i.skuId).padStart(4,"0")} `:"";
    const nm=(sku+(typeof i==="string"?i:i.nome)).substring(0,28);
    const q=typeof i==="string"?1:i.quantidade;
    const val=typeof i==="string"?0:i.preco*i.quantidade;
    doc.text(nm,4,y);doc.text(String(q),52,y,{align:"center"});doc.text(`R$${fmt(val)}`,76,y,{align:"right"});y+=4;
  });
  doc.text("- ".repeat(22),4,y);y+=5;
...
function renderHistorico() {
  ...
  [...filtradas].reverse().forEach(venda=>{
    ...
    venda.itens.forEach(item=>{
      const sku=item.skuId?`#${String(item.skuId).padStart(4,"0")} `:"";
      itensHtml+=typeof item==="string"?`<span class="item-historico">${item}</span>`:`<span class="item-historico">${sku}${item.nome} <strong>x${item.quantidade}</strong></span>`;
    });
    venda._idx=index;
    ...
    const acaoHtml=venda.cancelada
      ?`<span class="tag tag-cancelada">Cancelada</span>`
      :`<div style="display:flex;gap:6px;flex-wrap:wrap">
          <button class="btn-invalidar" onclick="cancelarVenda(${index})">Invalidar</button>
          <button class="btn-editar-venda" onclick="abrirEditarVenda(${index})">✏️ Editar</button>
          <button class="btn-copiar" onclick="abrirComprovanteHistorico(${index})">🖨 Comprovante</button>
        </div>`;
    ...
    card.innerHTML=`<div class="venda-header"><div class="venda-header-esq"><span class="venda-data">${venda.data}${hora}</span>${venda.cancelada?`<span class="tag tag-cancelada">Cancelada</span>`:""}</div><span class="venda-total">R$ ${fmt(venda.total)}</span></div>
      ${clienteLinha}${descHtml}<div class="venda-pagamento">${pagHtml}</div><div class="itens-historico">${itensHtml}</div><div style="margin-top:10px">${acaoHtml}</div>`;
    lista.appendChild(card);
  });
...
async function cancelarVenda(index){
  ...
  historico[index].cancelada=true;lsSet(chaveHistorico(usuarioAtual.email),historico);renderHistorico();
}
...
function confirmarSenhaAdmin() {
  ...
  hashSenha(senha).then(hash=>{
  if (hash!==usuarioAtual.senhaHash){mostrarToast("Senha incorreta.","erro");return;}
...
function _carregarFormEditarVenda() {
  ...
  editarTipoDesconto="val"; // valor fixo por padrão na edição
  // Desconto
...
function _adicionarItemEditar(prodId) {
  const p=produtos.find(p=>p.id===prodId); if (!p) return;
  const ex=editarItens.find(i=>i.nome===p.nome); if (ex){ex.quantidade++;} else editarItens.push({nome:p.nome,skuId:p.skuId,quantidade:1,preco:p.preco});
  renderEditarItens(); atualizarEditarTotal();
}
...
function renderEditarItens() {
  ...
  lista.innerHTML=editarItens.map((item,i)=>`
    <div class="editar-item-row">
      <span class="editar-item-nome">${item.skuId?`#${String(item.skuId).padStart(4,"0")} `:""}${item.nome}</span>
      <div style="display:flex;align-items:center;gap:6px;flex-shrink:0">
        <button class="btn-qtd sm" onclick="editarItemQtd(${i},-1)">−</button>
        <span style="min-width:24px;text-align:center;font-family:var(--mono);font-size:14px">${item.quantidade}</span>
        <button class="btn-qtd sm" onclick="editarItemQtd(${i},+1)">+</button>
        <span style="font-family:var(--mono);font-size:13px;color:var(--green-dark);min-width:70px;text-align:right">R$ ${fmt(item.preco*item.quantidade)}</span>
        <button class="carrinho-item-del" onclick="removerItemEditar(${i})">✕</button>
      </div>
    </div>`).join("");
}

function editarItemQtd(i,delta){
  editarItens[i].quantidade=Math.max(1,editarItens[i].quantidade+delta);
  renderEditarItens();atualizarEditarTotal();
}
function removerItemEditar(i){editarItens.splice(i,1);renderEditarItens();atualizarEditarTotal();}

function setEditTipoDesconto(tipo){
  editarTipoDesconto=tipo;
  ["edit-desc-tipo-pct","edit-desc-tipo-val"].forEach(id=>document.getElementById(id)?.classList.remove("ativo"));
  document.getElementById(`edit-desc-tipo-${tipo}`)?.classList.add("ativo");
  document.getElementById("editarDescontoSufixo").textContent=tipo==="pct"?"%":"R$";
  atualizarEditarTotal();
}

function toggleEditPagamento(tipo){
  const idx=editarPagamentos.indexOf(tipo);
  if (idx===-1){editarPagamentos.push(tipo);}
  else{if(editarPagamentos.length===1){mostrarToast("Ao menos uma forma necessária.","erro");return;}editarPagamentos.splice(idx,1);delete editarSplit[tipo];}
  ["dinheiro","cartao","pix"].forEach(t=>{const b=document.getElementById(`edit-pag-${t}`);if(b)b.classList.toggle("ativo",editarPagamentos.includes(t));});
  renderEditSplit();atualizarEditarTotal();
}

function _calcEditSubtotal(){return editarItens.reduce((a,i)=>a+i.preco*i.quantidade,0);}
function _calcEditDesconto(sub){
  const v=parseFloat(document.getElementById("editarDescontoInput")?.value)||0;
  if (v<=0) return 0;
  return editarTipoDesconto==="pct"?Math.min(sub,sub*(v/100)):Math.min(sub,v);
}
function _calcEditTotal(){const s=_calcEditSubtotal(),d=_calcEditDesconto(s);return Math.max(0,s-d);}

function atualizarEditarTotal(){
  const t=_calcEditTotal();
  const el=document.getElementById("editarTotalPreview");if(el)el.textContent=`R$ ${fmt(t)}`;
  if(editarPagamentos.length>1)renderEditSplit();
}
...
function salvarEdicaoVenda(){
  ...
  if (editarPagamentos.length>1){
    const soma=editarPagamentos.slice(0,-1).reduce((a,t)=>a+(parseFloat(editarSplit[t])||0),0);
    if(soma>total+.005){mostrarToast("⚠️ A soma dos pagamentos ultrapassa o total.","erro");return;}
  }
  const pagamentos=getSplitFinal(editarPagamentos,editarSplit,total);
  const v=historico[editarVendaIdx];
  historico[editarVendaIdx]={...v,itens:editarItens,subtotal:sub,desconto:desc,total,pagamentos,pagamento:editarPagamentos[0]};
  lsSet(chaveHistorico(usuarioAtual.email),historico);
...
function renderDashboard() {
  ...
  document.getElementById("kpiGrid").innerHTML=`
    <div class="kpi-card"><div class="kpi-icon">💰</div><div class="kpi-label">Receita hoje</div><div class="kpi-value green">R$ ${fmt(rh)}</div></div>
    <div class="kpi-card"><div class="kpi-icon">🛒</div><div class="kpi-label">Pedidos hoje</div><div class="kpi-value">${vh.length}</div></div>
    <div class="kpi-card"><div class="kpi-icon">📦</div><div class="kpi-label">Produtos</div><div class="kpi-value">${produtos.length}</div></div>
    <div class="kpi-card"><div class="kpi-icon">📈</div><div class="kpi-label">Receita total</div><div class="kpi-value green">R$ ${fmt(rt)}</div></div>`;
...
function renderRelatorios() {
  ...
  document.getElementById("relKpiGrid").innerHTML=`
    <div class="kpi-card"><div class="kpi-icon">💰</div><div class="kpi-label">Receita</div><div class="kpi-value green">R$ ${fmt(receita)}</div></div>
    <div class="kpi-card"><div class="kpi-icon">🛒</div><div class="kpi-label">Pedidos</div><div class="kpi-value">${pedidos}</div></div>
    <div class="kpi-card"><div class="kpi-icon">🎯</div><div class="kpi-label">Ticket médio</div><div class="kpi-value">R$ ${fmt(ticket)}</div></div>
    <div class="kpi-card"><div class="kpi-icon">🏷️</div><div class="kpi-label">Descontos</div><div class="kpi-value">R$ ${fmt(descontos)}</div></div>`;
  const pp={dinheiro:0,cartao:0,pix:0};
  ...
  listaEl.innerHTML=[...filtradas].reverse().map(venda=>{
    ...
    return `<div class="venda-export-card"><div class="venda-export-info"><strong>${venda.data}${hora}</strong> · ${itensTexto} · <strong>R$ ${fmt(venda.total)}</strong></div>
      <div style="display:flex;gap:6px;flex-wrap:wrap">
        <button class="btn-copiar" onclick="abrirComprovanteHistorico(${idx})">🖨 Comprovante</button>
        <button class="btn-copiar" onclick="copiarPedido(${idx})">📋 Copiar</button>
      </div></div>`;
  }).join("");
}
...
function exportarRelPDF() {
  ...
  const loja=lojaConfigAtual.nome||"HubVendasPro";
  const verde=[0,191,99], preto=[13,13,13], cinza=[120,120,120];
  let y=15;
  const pgW=210, mg=15, cW=pgW-mg*2;
  ...
  doc.text("Relatorio de Vendas", mg, 20);
  const periodoLabel = filtroRelatorio==="hoje"?"Hoje":filtroRelatorio==="semana"?"Ultimos 7 dias":filtroRelatorio==="mes"?"Ultimos 30 dias":filtroRelatorio==="tudo"?"Todo o periodo":"Periodo personalizado";
  doc.text(periodoLabel+" - Gerado em "+hojeStr(), pgW-mg, 20, {align:"right"});
  y=38;

  // KPIs
  const receita=filtradas.reduce((a,v)=>a+v.total,0);
  const pedidos=filtradas.length;
  const ticket=pedidos?receita/pedidos:0;
  const descontos=filtradas.reduce((a,v)=>a+(v.desconto||0),0);
  const kpis=[
    {label:"Receita Total", value:"R$ "+fmt(receita), cor:[0,191,99]},
    {label:"Pedidos", value:String(pedidos), cor:[13,13,13]},
    {label:"Ticket Medio", value:"R$ "+fmt(ticket), cor:[13,13,13]},
    {label:"Descontos", value:"R$ "+fmt(descontos), cor:[229,57,53]}
  ];
  const kW=(cW-9)/4;
  kpis.forEach(function(k,i){
    const x=mg+i*(kW+3);
    doc.setFillColor(248,248,248); doc.roundedRect(x,y,kW,20,2,2,"F");
    doc.setDrawColor(220,220,220); doc.roundedRect(x,y,kW,20,2,2,"S");
    doc.setFont("Helvetica","bold"); doc.setFontSize(9); doc.setTextColor(120,120,120);
    doc.text(k.label,x+kW/2,y+7,{align:"center"});
    doc.setFont("Helvetica","bold"); doc.setFontSize(11); doc.setTextColor(k.cor[0],k.cor[1],k.cor[2]);
  ...
  doc.text("Receita por Forma de Pagamento", mg, y); y+=6;
  const pp={dinheiro:0,cartao:0,pix:0};
  ...
  const tpp=Object.values(pp).reduce((a,b)=>a+b,0)||1;
  const pags=[{key:"dinheiro",label:"Dinheiro",cor:[16,185,129]},{key:"cartao",label:"Cartao",cor:[37,99,235]},{key:"pix",label:"Pix",cor:[13,148,136]}];
  pags.forEach(function(p){
    const pct=Math.round((pp[p.key]/tpp)*100);
    const bW=Math.max(1,(cW-60)*pp[p.key]/tpp);
    doc.setFont("Helvetica","normal"); doc.setFontSize(9); doc.setTextColor(120,120,120);
    doc.text(p.label,mg,y+3);
    doc.setFillColor(230,230,230); doc.roundedRect(mg+36,y-2,cW-60,5,2,2,"F");
    doc.setFillColor(p.cor[0],p.cor[1],p.cor[2]); doc.roundedRect(mg+36,y-2,bW,5,2,2,"F");
    doc.setTextColor(13,13,13); doc.text("R$ "+fmt(pp[p.key])+" ("+pct+"%)",pgW-mg,y+3,{align:"right"});
    y+=10;
  });
  y+=4;
  ...
  // Top produtos
  doc.setFont("Helvetica","bold"); doc.setFontSize(11); doc.setTextColor(13,13,13);
  doc.text("Top Produtos", mg, y); y+=7;
  const ct={};
  filtradas.forEach(function(v){v.itens.forEach(function(i){const nome=typeof i==="string"?i:i.nome,qtd=typeof i==="string"?1:i.quantidade,val=typeof i==="string"?0:i.preco*i.quantidade;if(!ct[nome])ct[nome]={qtd:0,val:0};ct[nome].qtd+=qtd;ct[nome].val+=val;});});
  const top=Object.entries(ct).sort((a,b)=>b[1].qtd-a[1].qtd).slice(0,5);
  if (top.length) {
    doc.setFillColor(0,191,99); doc.rect(mg,y-4,cW,7,"F");
    doc.setFont("Helvetica","bold"); doc.setFontSize(9); doc.setTextColor(255,255,255);
    doc.text("#",mg+3,y); doc.text("Produto",mg+12,y); doc.text("Qtd",mg+cW-30,y,{align:"right"}); doc.text("Receita",mg+cW,y,{align:"right"});
    y+=5;
    top.forEach(function([nome,d],i){
      if (i%2===0){doc.setFillColor(248,248,248);doc.rect(mg,y-4,cW,7,"F");}
      doc.setFont("Helvetica","bold"); doc.setFontSize(9); doc.setTextColor(0,191,99); doc.text(String(i+1),mg+3,y);
      doc.setFont("Helvetica","normal"); doc.setTextColor(13,13,13); doc.text(nome.substring(0,38),mg+12,y);
      doc.text(String(d.qtd)+" un.",mg+cW-30,y,{align:"right"});
      doc.setFont("Helvetica","bold"); doc.setTextColor(0,191,99); doc.text("R$ "+fmt(d.val),mg+cW,y,{align:"right"});
      y+=7;
    });
  } else { doc.setFont("Helvetica","normal"); doc.setFontSize(9); doc.setTextColor(120,120,120); doc.text("Sem dados.",mg,y); y+=6; }
  y+=6;
  ...
  if (clientes.length) {
    ...
    doc.setFont("Helvetica","bold"); doc.setFontSize(11); doc.setTextColor(13,13,13);
    doc.text("Clientes Cadastrados ("+clientes.length+")", mg, y); y+=7;
    doc.setFillColor(0,191,99); doc.rect(mg,y-4,cW,7,"F");
    doc.setFont("Helvetica","bold"); doc.setFontSize(9); doc.setTextColor(255,255,255);
    doc.text("Nome",mg+3,y); doc.text("Telefone",mg+70,y); doc.text("CPF",mg+110,y); doc.text("Email",mg+145,y);
    y+=5;
    clientes.slice(0,20).forEach(function(c,i){
      if (y>270){doc.addPage();y=15;}
      if (i%2===0){doc.setFillColor(248,248,248);doc.rect(mg,y-4,cW,7,"F");}
      doc.setFont("Helvetica","normal"); doc.setFontSize(8); doc.setTextColor(13,13,13);
      doc.text((c.nome||"-").substring(0,28),mg+3,y);
      doc.text((c.telefone||"-").substring(0,18),mg+70,y);
      doc.text((c.cpf||"-").substring(0,18),mg+110,y);
      doc.text((c.email||"-").substring(0,24),mg+145,y);
      y+=7;
    });
    if (clientes.length>20){doc.setFont("Helvetica","italic");doc.setFontSize(8);doc.setTextColor(120,120,120);y+=2;doc.text("... e mais "+(clientes.length-20)+" clientes.",mg,y);y+=5;}
    y+=4;
  }
  ...
  doc.setFont("Helvetica","bold"); doc.setFontSize(11); doc.setTextColor(13,13,13);
  doc.text("Vendas do Periodo", mg, y); y+=7;
  doc.setFillColor(0,191,99); doc.rect(mg,y-4,cW,7,"F");
  doc.setFont("Helvetica","bold"); doc.setFontSize(9); doc.setTextColor(255,255,255);
  doc.text("Data",mg+3,y); doc.text("Cliente",mg+28,y); doc.text("Itens",mg+70,y); doc.text("Desconto",mg+130,y,{align:"right"}); doc.text("Total",mg+cW,y,{align:"right"});
  y+=5;
  [...filtradas].reverse().forEach(function(v,i){
    if (y>270){doc.addPage();y=15;}
    if (i%2===0){doc.setFillColor(248,248,248);doc.rect(mg,y-4,cW,7,"F");}
    const itensStr=v.itens.map(it=>typeof it==="string"?it:(it.nome+" x"+it.quantidade)).join(", ").substring(0,38);
    doc.setFont("Helvetica","normal"); doc.setFontSize(8); doc.setTextColor(13,13,13);
    doc.text((v.data+(v.hora?" "+v.hora:"")),mg+3,y);
    doc.text((v.clienteNome||"-").substring(0,18),mg+28,y);
    doc.text(itensStr,mg+70,y);
    if (v.desconto>0){doc.setTextColor(229,57,53);doc.text("-R$ "+fmt(v.desconto),mg+130,y,{align:"right"});doc.setTextColor(13,13,13);}
    doc.setFont("Helvetica","bold"); doc.setTextColor(0,191,99);
    doc.text("R$ "+fmt(v.total),mg+cW,y,{align:"right"});
    y+=7;
  });

  // Footer em todas as pages
  const totalPgs=doc.getNumberOfPages();
  for(let i=1;i<=totalPgs;i++){
    doc.setPage(i);
    doc.setFillColor(248,248,248); doc.rect(0,285,pgW,12,"F");
    doc.setFont("Helvetica","normal"); doc.setFontSize(8); doc.setTextColor(120,120,120);
    doc.text("HubVendasPro - Relatorio gerado em "+hojeStr(), mg, 292);
    doc.text("Pagina "+i+" de "+totalPgs, pgW-mg, 292, {align:"right"});
  }
...
function renderConfiguracoes() {
  ...
  carregarImagem(`avatar_${usuarioAtual.email}`).then(av=>{
...
function definirTema(tema){document.documentElement.setAttribute("data-theme",tema);if(usuarioAtual)lsSet(chaveTema(usuarioAtual.email),tema);document.getElementById("tema-claro")?.classList.toggle("ativo",tema==="light");document.getElementById("tema-escuro")?.classList.toggle("ativo",tema==="dark");}
function trocarSenha(){
  ...
  hashSenha(at).then(ha=>{
    if(ha!==usuarioAtual.senhaHash){mostrarToast("Senha atual incorreta.","erro");return;}
    if(nv===at){mostrarToast("A nova senha deve ser diferente.","erro");return;}
    hashSenha(nv).then(hn=>{
      const users=lsGet(chaveUsuarios(),[]),idx=users.findIndex(u=>u.email===usuarioAtual.email);
      if(idx!==-1){users[idx].senhaHash=hn;lsSet(chaveUsuarios(),users);}
      usuarioAtual.senhaHash=hn;lsSet("usuarioLogado",{email:usuarioAtual.email,senhaHash:hn});
      ["senhaAtual","novaSenhaConfig","confirmarSenhaConfig"].forEach(id=>{const el=document.getElementById(id);if(el)el.value="";});

...
async function salvarImagem(k, b64) {
  try {
    ...
    // Se for avatar, também sobe pro Firebase Storage
    if (k.startsWith("avatar_") && usuarioAtual) {
      const sRef = storageRef(storage, `avatars/${usuarioAtual.uid}`);
      await uploadString(sRef, b64, "data_url").catch(()=>{});
    }
    return true;
  } catch { return false; }
}
async function carregarImagem(k) {
  try {
    const idb = await abrirDB();
    const local = await new Promise(res => { const req = idb.transaction("images","readonly").objectStore("images").get(k); req.onsuccess = () => res(req.result||null); req.onerror = () => res(null); });
    if (local) return local;
    // Se for avatar e não tiver local, tenta baixar do Storage
    if (k.startsWith("avatar_") && usuarioAtual) {
      try {
        const sRef = storageRef(storage, `avatars/${usuarioAtual.uid}`);
        ...
        return new Promise(res => { const r=new FileReader(); r.onload=e=>{ salvarImagem(k,e.target.result); res(e.target.result); }; r.readAsDataURL(blob); });
      } catch { return null; }
    }
    return null;
  } catch { return null; }

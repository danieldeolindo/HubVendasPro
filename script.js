...
const ICONE_PAGAMENTO_HTML = { dinheiro: `${ic("cash")} Dinheiro`, cartao: `${ic("card")} Cartão`, pix: `${ic("zap")} Pix` };
const MENU_PADRAO = [
  { id: "dashboard",     icon: ic("bar-chart"), label: "Dashboard",    bnav: "Dash"      },
  { id: "estoque",       icon: ic("package"),   label: "Estoque",      bnav: "Estoque"   },
  { id: "vendas",        icon: ic("cart"),      label: "Vendas",       bnav: "Vendas"    },
  { id: "admin",         icon: ic("package"),   label: "Produtos",     bnav: "Produtos"  },
  { id: "clientes",      icon: ic("users"),     label: "Clientes",     bnav: "Clientes"  },
  { id: "historico",     icon: ic("clipboard"), label: "Histórico",    bnav: "Histórico" },
  { id: "atendimento",   icon: ic("signal"),    label: "Atendimento RT", bnav: "Atend.RT"},
  { id: "configuracoes", icon: ic("settings"),  label: "Configurações", bnav: "Config"   },
];
...
function parseDDMMYYYY(valor) {
  ...
  if (/^\d{2}\/\d{2}\/\d{4}$/.test(texto)) {
    ...
    partes = texto.slice(0, 10).split("-").map(Number);
...
function normalizarTelefoneWhatsApp(v) {
...
function vendaNoIntervalo(v,{de,ate}) {
  ...
  const inicio = de ? new Date(de.getFullYear(), de.getMonth(), de.getDate()) : null;
  const fim = ate ? new Date(ate.getFullYear(), ate.getMonth(), ate.getDate()) : null;
  if (inicio&&d<inicio) return false; if (fim&&d>fim) return false; return true;
}
...
function renderNavs(paginaAtiva) {
  ...
  if (bn) {
    bn.innerHTML="";
    const itensMobile=items.filter(i=>i.id!=="configuracoes");
    const itensFixos=itensMobile.slice(0,4);
    const itensMais=[...itensMobile.slice(4), MENU_PADRAO.find(item=>item.id==="configuracoes")].filter(Boolean);
    ...
    const menuAtivo=itensMais.some(item=>item.id===pagina);
    const mais=document.createElement("button"); mais.className=`bnav-item bnav-more-trigger${menuAtivo?" active":""}`; mais.id="bnav-mobile-more";
    mais.innerHTML=`<span>${ic("menu")}</span><span>Mais</span>`; mais.onclick=toggleMobileMoreMenu; bn.appendChild(mais);
    const menu=document.getElementById("mobileMoreMenu");
    if (menu) {
      menu.querySelectorAll(".mobile-more-option").forEach(option=>option.remove());
      itensMais.forEach(item=>{
        const option=document.createElement("button"); option.type="button"; option.className="mobile-more-option"; option.dataset.mobilePage=item.id;
        option.innerHTML=`<span>${item.icon}</span><span>${item.label}</span>`;
        option.addEventListener("click",()=>{fecharMobileMoreMenu();navegarPara(item.id);}); menu.appendChild(option);
      });
    }
    initDragBottomNav();
  }
...
/* ─────────────────────────────────────────
   AUTH — SUPABASE
───────────────────────────────────────── */
function toggleSenha(id,el) { const i=document.getElementById(id); if (i.type==="password"){i.type="text";el.innerHTML=ic("eye-off");}else{i.type="password";el.innerHTML=ic("eye");} }
...
async function salvarCliente() {
  ...
  const end=document.getElementById("clienteEndereco").value.trim();
    if (!nome && !tel && !cpf && !email && !end) {
      mostrarToast("Preencha ao menos um campo.", "erro");
      return;
    }
  ...
  limparCamposCliente(); fecharFormularioCliente(); renderClientes(); popularSelectClientes();
}
...
function fecharFormularioCliente() {
  if (editandoClienteId === null) document.getElementById("clienteFormCard")?.classList.add("hidden");
}
...
function renderClientes() {
  ...
  if (!filtrados.length) { lista.innerHTML=`<div class="empty-state"><span>${ic("users")}</span>${busca?"Nenhum cliente encontrado.":"Nenhum cliente cadastrado ainda."}</div>`; return; }
    ...
    <div class="cliente-card">
      ...
      <div class="cliente-info">
        ...
        <div class="cliente-detalhes">
          ${c.telefone?`<span>${ic("smartphone")} ${c.telefone}</span>`:""}
          ${c.cpf?`<span>${ic("id-card")} ${c.cpf}</span>`:""}
          ${c.email?`<span>${ic("mail")} ${c.email}</span>`:""}
          ${c.endereco?`<span>${ic("map-pin")} ${c.endereco}</span>`:""}
        </div>
      </div>
      <div class="acoes-admin" style="flex-shrink:0">
        <button class="btn-sm btn-sm-edit" onclick="editarCliente(${c.id})">${ic("edit")} Editar</button>
        <button class="btn-sm btn-sm-del"  onclick="excluirCliente(${c.id})">${ic("trash")}</button>
      </div>
    </div>`).join("");
}
...
function renderCarrinho() {
  ...
  itensEl.innerHTML=itensCarrinho.map(([id,qtd])=>{
    ...
    return `<div class="carrinho-item">
      ...
      <button class="carrinho-item-del" onclick="removerDoCarrinho(${p.id})">${ic("x")}</button>
    </div>`;
  }).join("");
  footerEl.innerHTML=`
...
function _renderSplitHTML(container, formas, splits, total, cbInput, cbAviso) {
  ...
  formas.forEach((tipo,i) => {
    ...
    if (isUlt) { const soma=formas.filter((_,j)=>j!==i).reduce((a,t)=>a+(parseFloat(splits[t])||0),0), resto=Math.max(0,total-soma); html+=`<div class="pagamento-split-row"><span class="pagamento-split-label">${ICONE_PAGAMENTO_HTML[tipo]}</span><span class="pagamento-split-resto" id="split-resto-${tipo}">R$ ${fmt(resto)} (restante)</span></div>`; }
    else { const v=splits[tipo]!==undefined?splits[tipo]:""; html+=`<div class="pagamento-split-row"><span class="pagamento-split-label">${ICONE_PAGAMENTO_HTML[tipo]}</span><input type="number" class="pagamento-split-input" min="0" step="0.01" id="split-input-${tipo}" value="${v}" placeholder="0,00" oninput="${cbInput}('${tipo}')"></div>`; }
  });
...
function _atualizarResto(formas, splits, total, avisoId) {
  ...
  aviso.innerHTML=soma>total+.005?`<span class="pagamento-split-aviso">A soma ultrapassa o total (R$ ${fmt(total)})</span>`:`<span class="pagamento-split-ok">${ic("check")} R$ ${fmt(soma)} + R$ ${fmt(total-soma)} = R$ ${fmt(total)}</span>`;
}
...
function renderTagsPagamento(venda) {
  if (venda.pagamentos&&Object.keys(venda.pagamentos).length) {
    const tags=Object.entries(venda.pagamentos).filter(([,v])=>v>0).map(([t,v])=>`<span class="tag-pag-valor ${t}">${ICONE_PAGAMENTO_HTML[t]}: R$ ${fmt(v)}</span>`).join("");
    return `<div class="tags-pag-multi">${tags}</div>`;
  }
  const p=venda.pagamento||"dinheiro";
  return `<span class="tag-pag ${p}">${ICONE_PAGAMENTO_HTML[p]}</span>`;
}
...
function _renderProdutos() {
  ...
  if (!produtos.length){lista.innerHTML=`<div class="empty-state"><span>${ic("package")}</span>Nenhum produto cadastrado.<br>Vá em <strong>Produtos</strong> para adicionar.</div>`;return;}
  ...
  if (!filtrados.length){lista.innerHTML=`<div class="empty-state"><span>${ic("search")}</span>Nenhum produto encontrado.</div>`;return;}
...
function renderProdutosAdmin() {
  ...
  if (!produtos.length){lista.innerHTML=`<div class="empty-state"><span>${ic("package")}</span>Nenhum produto ainda.</div>`;return;}
  ...
  filtrados.forEach(prod=>{
    ...
    card.innerHTML=`${imgTag}<span class="prod-sku">#${String(prod.skuId).padStart(4,"0")}</span>${catTag}<p class="prod-nome">${prod.nome}</p><p class="prod-preco">R$ ${fmt(prod.preco)}</p><div class="prod-meta"><span>Custo: R$ ${fmt(prod.custo||0)}</span><span>Estoque: ${prod.estoque||0}</span></div>
      <div class="acoes-admin"><button class="btn-sm btn-sm-edit" onclick="editarProduto(${prod.id})">${ic("edit")} Editar</button><button class="btn-sm btn-sm-del" onclick="excluirProduto(${prod.id})">${ic("trash")} Excluir</button></div>`;
    lista.appendChild(card);
    if (!prod.fotoUrl && prod.fotoKey){carregarImagem(prod.fotoKey).then(b64=>{if(b64){prod._fotoCache=b64;lista.querySelectorAll(`img[data-foto-key="${prod.fotoKey}"]`).forEach(img=>img.src=b64);}});}
  });
}
...
function abrirNovoProduto(){editandoId=null;limparCampos();document.getElementById("adminFormLabel").textContent="Novo produto";document.getElementById("btnCancelarEdicao").style.display="inline-flex";document.getElementById("adminFormCard")?.classList.remove("hidden");document.getElementById("nomeProduto")?.focus();}
function cancelarEdicao(){editandoId=null;limparCampos();document.getElementById("adminFormLabel").textContent="Novo produto";document.getElementById("btnCancelarEdicao").style.display="none";document.getElementById("adminFormCard")?.classList.add("hidden");}
...
async function finalizarPedido() {
  ...
  if (pagamentosSelecionados.length>1){
    ...
    if (soma>total+.005){mostrarToast("A soma dos pagamentos ultrapassa o total.","erro");return;}
  }
...
function montarHTMLComprovante(venda) {
  ...
  const pagTxt=venda.pagamentos&&Object.keys(venda.pagamentos).length>1
    ?Object.entries(venda.pagamentos).filter(([,v])=>v>0).map(([t,v])=>`${ICONE_PAGAMENTO_HTML[t]}: R$ ${fmt(v)}`).join("<br>")
    :ICONE_PAGAMENTO_HTML[venda.pagamento||"dinheiro"];
...
function renderHistorico() {
  ...
  const busca=(document.getElementById("buscaHistorico")?.value||"").toLowerCase().trim();
  const correspondeBusca = v => {
    if (!busca) return true;
    const cliente=(v.clienteNome||v.cliente_nome||"").toLowerCase();
    const itens=(v.itens||[]).map(item=>typeof item==="string"?item:item.nome||"").join(" ").toLowerCase();
    return cliente.includes(busca)||itens.includes(busca);
  };
  const filtradasVendas = tipoHistorico === "art" ? [] : historico.filter(v=>vendaNoIntervalo(v,iv)&&correspondeBusca(v));
  const filtradasArt = tipoHistorico === "vendas" ? [] : pedidosRT.filter(v=>vendaNoIntervalo(v,iv)&&correspondeBusca(v));
  const filtradas = [...filtradasVendas, ...filtradasArt].sort((a,b)=>(parseDDMMYYYY(b.data)?.getTime()||0)-(parseDDMMYYYY(a.data)?.getTime()||0));
  if (!filtradas.length){lista.innerHTML=`<div class="empty-state"><span>${ic("clipboard")}</span>Nenhuma venda neste período.</div>`;document.getElementById("balanco").innerText="0,00";return;}
  filtradas.forEach(venda=>{
    if (venda.origem === "art") {
      if (!venda.cancelada && venda.status === "atendido") balanco += venda.total || 0;
    ...
    const clienteLinha=venda.clienteNome?`<p class="venda-cliente">${ic("user")} ${venda.clienteNome}</p>`:"";
    const acaoHtml=venda.cancelada?`<span class="tag tag-cancelada">Cancelada</span>`:`<div style="display:flex;gap:6px;flex-wrap:wrap"><button class="btn-invalidar" onclick="cancelarVenda(${index})">Cancelar venda</button><button class="btn-editar-venda" onclick="abrirEditarVenda(${index})">${ic("edit")} Editar</button><button class="btn-copiar" onclick="abrirComprovanteHistorico(${index})">${ic("printer")} Comprovante</button></div>`;
...
async function reverterEstoqueVenda(venda) {
  ...
  for (const item of venda.itens) {
    ...
    const skuId = Number(item.skuId ?? item.id ?? 0);
    const produto = produtos.find(p =>
      (Number(p.skuId) === skuId && skuId > 0) ||
      (Number(p.id) === Number(item.id) && Number(item.id) > 0)
    );

    if (!produto) continue;

    produto.estoque = Math.max(0, (Number(produto.estoque) || 0) + quantidade);
    await fbSalvarProduto(produto);
  }
}

async function cancelarVenda(index){
  const venda = historico[index];
  if (!venda || venda.cancelada) return;

  const ok=await confirmar("Deseja invalidar esta venda?");if(!ok)return;

  venda.cancelada=true;
  await reverterEstoqueVenda(venda);

  if (venda.firestoreId) await fbAtualizarVenda(venda.firestoreId,{cancelada:true});
...
async function salvarEdicaoVenda(){
  ...
  if (editarPagamentos.length>1){const soma=editarPagamentos.slice(0,-1).reduce((a,t)=>a+(parseFloat(editarSplit[t])||0),0);if(soma>total+.005){mostrarToast("A soma dos pagamentos ultrapassa o total.","erro");return;}}
...
function renderDashboard() {
  ...
  document.getElementById("kpiGrid").innerHTML=`<div class="kpi-card"><div class="kpi-icon">${ic("wallet")}</div><div class="kpi-label">Faturamento</div><div class="kpi-value green">R$ ${fmt(rt)}</div></div><div class="kpi-card"><div class="kpi-icon">${ic("trending-up")}</div><div class="kpi-label">Lucro líquido</div><div class="kpi-value ${lucro<0?"red":"green"}">R$ ${fmt(lucro)}</div></div><div class="kpi-card"><div class="kpi-icon">${ic("cart")}</div><div class="kpi-label">Vendas concluídas</div><div class="kpi-value">${vt.length}</div></div><div class="kpi-card"><div class="kpi-icon">${ic("target")}</div><div class="kpi-label">Ticket médio</div><div class="kpi-value">R$ ${fmt(ticket)}</div></div><div class="kpi-card"><div class="kpi-icon">${ic("x-circle")}</div><div class="kpi-label">Vendas canceladas</div><div class="kpi-value">${canceladas.length}</div></div><div class="kpi-card"><div class="kpi-icon">${ic("tag")}</div><div class="kpi-label">Descontos</div><div class="kpi-value">R$ ${fmt(descontos)}</div></div>`;
  ...
  document.getElementById("dashPagamentos").innerHTML=[{key:"dinheiro",label:`${ic("cash")} Dinheiro`},{key:"cartao",label:`${ic("card")} Cartão`},{key:"pix",label:`${ic("zap")} Pix`}].map(({key,label})=>{const w=Math.round((pp[key]/tpp)*100);return `<div class="pag-row"><span class="pag-nome">${label}</span><div class="pag-bar-wrap"><div class="pag-bar" style="width:${w}%"></div></div><span class="pag-val">R$ ${fmt(pp[key])}</span></div>`;}).join("");
  const qtdVendas=vt.filter(v=>v.origem!=="art").length, qtdArt=vt.filter(v=>v.origem==="art").length;
  document.getElementById("dashOrigem").innerHTML=`<div class="pag-row"><span class="pag-nome">${ic("cart")} Vendas</span><div class="pag-bar-wrap"><div class="pag-bar" style="width:${Math.round((qtdVendas/(vt.length||1))*100)}%"></div></div><span class="pag-val">${qtdVendas} pedidos</span></div><div class="pag-row"><span class="pag-nome">${ic("signal")} ART</span><div class="pag-bar-wrap"><div class="pag-bar" style="width:${Math.round((qtdArt/(vt.length||1))*100)}%"></div></div><span class="pag-val">${qtdArt} pedidos</span></div>`;
}

function renderEstoque() {
  const lista=document.getElementById("estoqueLista"), kpis=document.getElementById("estoqueKpiGrid");
  if (!lista||!kpis) return;
  const totalProdutos=produtos.length;
  const totalUnidades=produtos.reduce((s,p)=>s+(Number(p.estoque)||0),0);
  const semEstoque=produtos.filter(p=>(Number(p.estoque)||0)<=0).length;
  const baixoEstoque=produtos.filter(p=>(Number(p.estoque)||0)>0&&(Number(p.estoque)||0)<=5).length;
  const custoEstoque=produtos.reduce((s,p)=>s+(Number(p.custo)||0)*(Number(p.estoque)||0),0);
  const valorEstoque=produtos.reduce((s,p)=>s+(Number(p.preco)||0)*(Number(p.estoque)||0),0);
  kpis.innerHTML=`<div class="kpi-card"><div class="kpi-icon">${ic("package")}</div><div class="kpi-label">Produtos cadastrados</div><div class="kpi-value">${totalProdutos}</div></div><div class="kpi-card"><div class="kpi-icon">${ic("hash")}</div><div class="kpi-label">Unidades em estoque</div><div class="kpi-value green">${totalUnidades}</div></div><div class="kpi-card"><div class="kpi-icon">${ic("alert-triangle")}</div><div class="kpi-label">Estoque baixo</div><div class="kpi-value">${baixoEstoque}</div></div><div class="kpi-card"><div class="kpi-icon">${ic("ban")}</div><div class="kpi-label">Sem estoque</div><div class="kpi-value red">${semEstoque}</div></div><div class="kpi-card"><div class="kpi-icon">${ic("wallet")}</div><div class="kpi-label">Custo do estoque</div><div class="kpi-value">R$ ${fmt(custoEstoque)}</div></div><div class="kpi-card"><div class="kpi-icon">${ic("tag")}</div><div class="kpi-label">Valor de venda</div><div class="kpi-value green">R$ ${fmt(valorEstoque)}</div></div>`;
  if (!produtos.length) { lista.innerHTML=`<div class="empty-state"><span>${ic("package")}</span>Nenhum produto cadastrado.</div>`; return; }
  const ordenados=[...produtos].sort((a,b)=>(Number(a.estoque)||0)-(Number(b.estoque)||0));
  lista.innerHTML=ordenados.map(p=>{
    const estoque=Number(p.estoque)||0, status=estoque<=0?"Sem estoque":estoque<=5?"Estoque baixo":"Disponível";
    const classe=estoque<=0?"sem-estoque":estoque<=5?"baixo":"disponivel";
    return `<div class="estoque-produto-row"><div class="estoque-produto-info"><strong>${p.nome}</strong><small>SKU #${String(p.skuId).padStart(4,"0")} · Custo R$ ${fmt(p.custo||0)} · Venda R$ ${fmt(p.preco)}</small></div><span class="estoque-quantidade">${estoque} un.</span><span class="estoque-status ${classe}">${status}</span><button class="btn-sm btn-sm-edit" onclick="editarProdutoDoEstoque(${p.id})">${ic("edit")} Editar</button></div>`;
  }).join("");
}

function editarProdutoDoEstoque(id) { navegarPara("admin"); setTimeout(()=>editarProduto(id),0); }
...
function renderRelatorios() {
  ...
  document.getElementById("relKpiGrid").innerHTML=`<div class="kpi-card"><div class="kpi-icon">${ic("wallet")}</div><div class="kpi-label">Receita</div><div class="kpi-value green">R$ ${fmt(receita)}</div></div><div class="kpi-card"><div class="kpi-icon">${ic("cart")}</div><div class="kpi-label">Pedidos</div><div class="kpi-value">${pedidos}</div></div><div class="kpi-card"><div class="kpi-icon">${ic("target")}</div><div class="kpi-label">Ticket médio</div><div class="kpi-value">R$ ${fmt(ticket)}</div></div><div class="kpi-card"><div class="kpi-icon">${ic("tag")}</div><div class="kpi-label">Descontos</div><div class="kpi-value">R$ ${fmt(descontos)}</div></div>`;
  const pp={dinheiro:0,cartao:0,pix:0};
  ...
  const tpp=Object.values(pp).reduce((a,b)=>a+b,0)||1;
  document.getElementById("relPagamentos").innerHTML=[{key:"dinheiro",label:`${ic("cash")} Dinheiro`},{key:"cartao",label:`${ic("card")} Cartão`},{key:"pix",label:`${ic("zap")} Pix`}].map(({key,label})=>{const w=Math.round((pp[key]/tpp)*100);return `<div class="pag-row"><span class="pag-nome">${label}</span><div class="pag-bar-wrap"><div class="pag-bar" style="width:${w}%"></div></div><span class="pag-val">R$ ${fmt(pp[key])}</span></div>`;}).join("");
...
function renderSelecaoCatalogoRT() {
  ...
  lista.innerHTML = produtos.map(p => `
    <label class="catalogo-check-item">
      ...
      <span class="catalogo-check-foto">${p.fotoUrl?`<img src="${p.fotoUrl}" alt="" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">` : ""}<span class="catalogo-check-placeholder" style="display:${p.fotoUrl?"none":"flex"}">${ic("package")}</span></span>
...
async function sincronizarPedidosRTPolling() {
  ...
  data.reverse().forEach(pedido => {
    ...
    mostrarToast("Novo pedido recebido!","notif");
...
function atualizarStatusNotificacoesPedidos() {
  ...
  botao.innerHTML = bloqueadas ? `${ic("bell")} Liberar nas configurações` : ativas ? `${ic("bell-off")} Desativar notificações` : `${ic("bell")} Ativar notificações`;
  ...
  if (!pushSubscription && "Notification" in window && Notification.permission === "granted") {
    ...
    try {
      if (registroServiceWorker) await registroServiceWorker.showNotification("Novo pedido recebido", opcoes);
        ...
        const notificacao = new Notification("Novo pedido recebido", opcoes);
        notificacao.onclick = () => { window.focus(); navegarPara("atendimento"); notificacao.close(); };
      }
    } catch (error) { console.warn("Não foi possível exibir a notificação:", error); }
  }
}
...
function renderPedidoRTCard(p) {
  ...
  const pagTxt = p.pagamentos && Object.keys(p.pagamentos).length>1
    ? Object.entries(p.pagamentos).filter(([,v])=>v>0).map(([t,v])=>`${ICONE_PAGAMENTO_HTML[t]}: R$ ${fmt(v)}`).join(" + ")
    : (ICONE_PAGAMENTO_HTML[p.pagamento]||p.pagamento);
  return `<div class="pedido-rt-card ${concluido?"atendido":""} ${cancelado?"cancelado":""}" id="pedidoRT-${p.id}">
    <div class="pedido-rt-topo">
      <div class="pedido-rt-identidade"><span class="pedido-rt-cliente">${ic("user")} ${p.cliente_nome||p.clienteNome||"Cliente"}</span><span class="tag tag-origem-art">Atendimento ART</span>${cancelado?`<span class="tag tag-cancelada">Cancelado</span>`:concluido?`<span class="tag tag-concluido">Concluído</span>`:`<span class="tag tag-pendente">Pendente</span>`}</div>
    ...
    <div class="pedido-rt-rodape">
      ...
      <span class="pedido-rt-tel">${ic("phone")} ${p.cliente_telefone||"—"}</span>
    </div>
    <div class="pedido-rt-acoes">
      <button class="btn-copiar btn-sm" onclick="abrirComprovanteRT('${p.id}')">${ic("printer")} Comprovante</button>
      ${p.cliente_telefone?`<button class="btn-ghost btn-sm" onclick="window.open('https://wa.me/${p.cliente_telefone.replace(/\\D/g,"")}','_blank')">${ic("message-circle")} Chamar no WhatsApp</button>`:""}
      ${cancelado?`<span class="pedido-status-text cancelado">Pedido cancelado</span>`:concluido?`<span class="pedido-status-text concluido">${ic("check")} Pedido concluído</span>`:`<button class="btn-primary btn-sm" onclick="marcarPedidoRTAtendido('${p.id}')">${ic("check")} Marcar como concluído</button><button class="btn-invalidar btn-sm" onclick="cancelarPedidoRT('${p.id}')">Cancelar pedido</button>`}
    </div>
  </div>`;
}
...
function atualizarResumoMinimizadoCatalogo() {
  ...
  resumo.innerHTML = `${ic("cart")} ${quantidade} ite${quantidade === 1 ? "m" : "ns"} · R$ ${fmt(totalCatalogo())}`;
...
async function iniciarModoCatalogo() {
  ...
  try {
    ...
    whatsappLojaPublico = normalizarTelefoneWhatsApp(loja.whatsapp);
...
function renderCatalogoProdutosPublico() {
  ...
  lista.innerHTML = catalogoProdutosPublicos.map(p => `
    <div class="card-produto catalogo-produto-card">
      ...
      ${(Number(p.estoque)||0) <= 0 ? `<p class="catalogo-sem-estoque">Sem estoque</p>` : `<p class="catalogo-estoque">Disponível</p>`}
      <div class="catalogo-qtd-row">
        <button class="btn-qtd sm" onclick="alterarQtdCatalogo('${p.id}',-1)" ${(Number(p.estoque)||0) <= 0 ? "disabled" : ""}>−</button>
...
function alterarQtdCatalogo(id, delta) {
  ...
  const nova = Math.min(estoque, Math.max(0, atual+delta));
  if (delta > 0 && nova === atual && estoque > 0) { mostrarToast("Quantidade maior que o estoque disponivel.", "erro"); return; }
...
function atualizarCupomCatalogo() {
  ...
  if (status) {
    status.innerHTML = !digitado ? "" : cupomCatalogoAplicado ? `${ic("check")} ${cupomCatalogo.percentual}% de desconto aplicado` : "Cupom inválido";
...
function atualizarResumoPagamentoCatalogo() {
  ...
  resumo.innerHTML = `<strong>Total do pedido: R$ ${fmt(total)}</strong><span>${pagamentosCatalogo.map(tipo => `${ICONE_PAGAMENTO_HTML[tipo]}: R$ ${fmt(pagamentos[tipo] || 0)}`).join(" + ")}</span><small>Soma dos pagamentos: R$ ${fmt(soma)}</small>`;
}
...
Object.assign(window, {
  ...
  // Produtos
  salvarProduto, editarProduto, abrirNovoProduto, excluirProduto, cancelarEdicao,
  atualizarNomeArquivo, removerFotoProduto, renderProdutosAdmin,
  // Clientes
  salvarCliente, editarCliente, abrirFormularioCliente, abrirNovoCliente, excluirCliente, cancelarEdicaoCliente, renderClientes,
  // Estoque
  renderEstoque, editarProdutoDoEstoque,
  // Formatadores
